  ...
  if (historyPoints.length === 0) {
    return (
      <div className="rounded-3xl border border-slate-200/80 bg-slate-50 p-6 text-center text-sm text-slate-600 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-300">
        {emptyMessage ?? getTranslation("exchangeChartLoading", language)}
      </div>
    );
  }
  ...
  return (
    <div className="rounded-3xl border border-slate-200/80 bg-white/90 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950/95">
      <div className="flex items-center justify-between gap-4">
        <div>
          <h3 className="text-lg font-semibold text-slate-950 dark:text-white">{getTranslation("exchangeChartTitle", language)}</h3>
          <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">{getTranslation("exchangeChartDescription", language).replace("{targetCurrency}", targetCurrency)}</p>
        </div>
